 ...
 */
function extractParenContent(signature) {
 ...
 */
function stripAnnotationsAndModifiers(paramDecl) {
    ...
    while (remaining.length > 0) {
        ...
        for (const modifier of PARAM_MODIFIERS) {
            if (trimmed.startsWith(modifier) &&
 ...
 */
function stripLeadingAnnotation(text) {
    // 跳过 @AnnotationName
    let i = 1; // skip '@'
 ...
 */
function splitByTopLevelComma(paramsStr) {
    const result = [];
    let current = "";
...
function parseReturnType(signature) {
    ...
    // 尝试匹配：修饰符... 返回类型 方法名(
    const match = /(?:public|private|protected|static|final|abstract|synchronized|default|native|\s)*\s*([\w<>\[\],\s?]+?)\s+\w+\s*\(/.exec(withoutGenericDecl);
    if (match?.[1]) {
        return match[1].trim();
    }
    // 备用方案：提取括号前的最后一个类型标识
 ...
 * 不能用简单正则 /<[^>]+>/ 因为泛型可能嵌套
 */
function removeMethodGenericDecl(signature) {
